...
class APPNP(GNNBase):
    ...
    def __init__(self, layer_size, activation, use_pp, dropout=0.5, norm='layer', train_size=None, n_linear=0, alpha=0):
        super(APPNP, self).__init__(layer_size, activation, use_pp, dropout, norm, n_linear)
        self.alpha = alpha
        for i in range(self.n_layers):
            if i < self.n_layers - self.n_linear:
                self.layers.append(APPNPLayer(layer_size[i], layer_size[i + 1], use_pp=use_pp))
            else:
                self.layers.append(nn.Linear(layer_size[i], layer_size[i + 1]))
            if i < self.n_layers - 1 and self.use_norm:
                if norm == 'layer':
    ...
    def forward(self, g, feat, in_deg=None):
        h = feat
        h = self.layers[0](g, h, in_deg, self.alpha, None)
        h = self.activation(h)
        h0 = h.clone()
        for i in range(1, self.n_layers):
            ...
            if i < self.n_layers - self.n_linear:
                if self.training and (i > 0 or not self.use_pp):
                    h = ctx.buffer.update(i, h)
                h = self.layers[i](g, h, in_deg, self.alpha, h0)
            else:
                h = self.layers[i](h)
