...
def get_boundary(node_dict, gpb):
    ...
    return boundary, re_index

def get_boundary_prob(node_dict, gpb, score, num_nodes_in, re_index):
    rank, size = dist.get_rank(), dist.get_world_size()
    device = 'cuda'
    boundary_prob = [None] * size
    ...
    for i in range(1, size):
        ...
        req = dist.isend(num_right, dst=right)
        dist.recv(num_left, src=left)
        v = score[belong_right[num_nodes_in:]]
        v = torch.sqrt(v)
        v = v / torch.sum(v)
       
        if dist.get_backend() == 'gloo':
            v = v.cpu()
            u = torch.zeros(num_left)
        else:
            u = torch.zeros(num_left,  device=device)
        req.wait()
        req = dist.isend(v, dst=right)
        dist.recv(u, src=left)

        boundary_prob[left] = u

        req.wait()
    
    for i in range(len(boundary_prob)):
        if boundary_prob[i] == None:
            continue
        else:
            boundary_prob[i] = boundary_prob[i][re_index[i]]

    return boundary_prob

def reget_recv_size(send_size):
    rank, size = dist.get_rank(), dist.get_world_size()
    recv_size = [0] * size
    device = 'cuda'
    ...
    for i in range(1, size):
        left = (rank - i + size) % size
        right = (rank + i) % size
        num_right = torch.tensor([send_size[right]])
        if dist.get_backend() == 'gloo':
            num_right = num_right.cpu()
            num_left = torch.tensor([0])
        else:
            num_left = torch.tensor([0], device=device)
        req = dist.isend(num_right, dst=right)
        u = dist.recv(num_left, src=left)
        recv_size[num_left] = u.item()
        req.wait()
