...
class Buffer(object):
    ...
    def __gloo_all_to_all(self, send_gpu, send_cpu, recv_cpu, recv_gpu, forward=True):
        ...
        for i in range(1, size):
            ...
            if forward:
                send_cpu[right].copy_(send_gpu[self._selected[right]] / self._ratio[right][self._index[right]])
            else:
        ...
        while not req.empty():
            ...
            if forward:
                ...
                send_gpu[self._selected[idx]] += recv_cpu[idx].cuda(non_blocking=True) / self._ratio[idx][self._index[idx]]
