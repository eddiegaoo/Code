...
def move_to_cuda(graph, in_graph, out_graph, node_dict, boundary, boundary_prob):
    rank, size = dist.get_rank(), dist.get_world_size()
    boundary_prob_numpy = [None] * size
    scaling = [None] * size
    for i in range(size):
        if i != rank:
            boundary[i] = boundary[i].cuda()
            boundary_prob_numpy[i] = boundary_prob[i].numpy()
    ...
    return graph, in_graph, out_graph, node_dict, boundary, boundary_prob_numpy
...
def get_send_size(boundary, rate, prob):
    rank, size = dist.get_rank(), dist.get_world_size()
    res, ratio = [], []
...
def precompute(part, graph, node_dict, boundary, model, gpb, pos):
    ...
    elif model == 'graphsage' or model == 'appnp':
...
def create_model(layer_size, args):
    if args.model == 'graphsage':
        ...
        return APPNP(layer_size, F.relu, norm=args.norm, use_pp=args.use_pp, dropout=args.dropout,
                train_size=args.n_train, n_linear=args.n_linear,alpha=0.2)


def select_node(boundary, send_size, prob):
    rank, size = dist.get_rank(), dist.get_world_size()
    index_selected = []
    selected = []
    for i in range(size):
        if i == rank:
            ...
            index_selected.append(None)
            continue
        b = boundary[i]
        idx = torch.as_tensor(np.random.choice(b.shape[0], send_size[i], replace=False, p=prob[i]), \
                              dtype=torch.long, device='cuda')
        index_selected.append(idx)
        selected.append(b[idx])
    return index_selected, selected
...
def get_score(graph, num_nodes_in, model_name):
    rank, size = dist.get_rank(), dist.get_world_size()
    if model_name != 'gat':
        degree_in = graph.in_degrees(range(num_nodes_in))
        degree_in_inv = 1 / degree_in

        degree_in_inv = degree_in_inv ** 2 
        value = degree_in_inv.unsqueeze(dim=1)
    else:
        value = torch.ones(num_nodes_in,1)
    boundary_edge = graph.out_edges(graph.nodes()[num_nodes_in:])
    
    temp_g = dgl.heterograph({('_U', '_E', '_V'): (boundary_edge[1], boundary_edge[0])})
    if temp_g.num_nodes('_U') < num_nodes_in:
        temp_g.add_nodes(num_nodes_in - temp_g.num_nodes('_U'), ntype = '_U')
    temp_g.nodes['_U'].data['h'] = value
    temp_g['_E'].update_all(fn.copy_src(src='h', out='m'),fn.sum(msg='m', out='h'),etype='_E')
    score = temp_g.nodes['_V'].data['h'][len(value):]
...
def run(graph, node_dict, gpb, args):
    ...
    score = get_score(graph, in_graph.num_nodes(), args.model)
    ...
    boundary, re_index = get_boundary(node_dict, gpb)
    ta = time.time()
    boundary_prob = get_boundary_prob(node_dict, gpb, score, in_graph.num_nodes(), re_index)
    tb = time.time()
    
    layer_size = get_layer_size(args.n_feat, args.n_hidden, args.n_class, args.n_layers)
                              
    graph, in_graph, out_graph, node_dict, boundary, boundary_prob_numpy = move_to_cuda(graph, in_graph, out_graph, node_dict, boundary, boundary_prob)
    ...
    send_size, ratio = get_send_size(boundary, args.sampling_rate, boundary_prob)
    ...
    for epoch in range(args.n_epochs):
        ...
        t0 = time.time()
        index_selected, selected = select_node(boundary, send_size, boundary_prob_numpy)
        one_hops = data_transfer(selected, recv_shape, tag=TransferTag.NODE, dtype=torch.long)
        ctx.buffer.set_selected(index_selected, selected)
        g = construct(in_graph, out_graph, pos, one_hops)

        model.train()

        if args.model == 'graphsage' or args.model == 'appnp':
